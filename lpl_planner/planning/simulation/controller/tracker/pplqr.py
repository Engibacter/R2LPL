...
class PPLQRTracker(AbstractTracker):
    """
    Implements a Pure-Pursuit for lateral and LQR for longitudinal tracker for a kinematic bicycle model.

    We decouple into two subsystems, longitudinal and lateral, with small angle approximations for linearization.
    The lateral subsystem uses a Pure-Pursuit controller, while the longitudinal subsystem uses an LQR controller.
    Parameters for the LQR controller are provided in the constructor.  The lateral controller uses a lookahead distance
    based on the current velocity and a lookahead time of self._tracking_horizon * self._discretization_time.  
    The lateral controller uses a lookahead distance based on the current velocity and a lookahead time of self._tracking_horizon * self._discretization_time.
