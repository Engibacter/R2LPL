...
from .control_utils import StateIndex, DynamicStateIndex

KINEMATIC_DTYPE = np.float32


def forward_integrate(
    init: npt.NDArray[np.float32],
...
class BatchKinematicBicycleModel:
    ...
    def get_state_dot(self, states: npt.NDArray[np.float32]) -> npt.NDArray[np.float32]:
        """
        ...
        """
        state_dots = np.zeros(states.shape, dtype=KINEMATIC_DTYPE)
    ...
    def _update_commands(
        ...
        command_states: npt.NDArray[np.float32],
        sampling_time: TimePoint,
    ) -> EgoState:
        """
        ...
        propagating_state: npt.NDArray[np.float32] = copy.deepcopy(states)
