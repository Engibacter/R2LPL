...
class BatchLQRTracker:
    ...
    def __init__(
        ...
        self._q_lateral = np.diag(np.asarray(q_lateral, dtype=CONTROL_DTYPE))
        self._r_lateral = np.diag(np.asarray(r_lateral, dtype=CONTROL_DTYPE))
    ...
    def track_trajectory(
        ...
        accel_cmds = np.zeros(batch_size, dtype=CONTROL_DTYPE)
        steering_rate_cmds = np.zeros(batch_size, dtype=CONTROL_DTYPE)
        ...
        active_idx = np.where(~should_stop_mask)[0]
        if active_idx.size > 0:
            accel_cmds[active_idx] = self._longitudinal_lqr_controller(
                initial_velocity[active_idx], reference_velocities[active_idx]
            )

            velocity_profiles = _generate_profile_from_initial_condition_and_derivatives(
                initial_condition=initial_velocity[active_idx],
                derivatives=np.repeat(
                    accel_cmds[active_idx, None], self._tracking_horizon, axis=-1
                ),
                discretization_time=self._discretization_time,
            )[:, : self._tracking_horizon]

            initial_velocity_pp, initial_lat_state_pp = self._compute_lookahead_lateral_error(
                initial_state=initial_states[active_idx],
                proposal_states=self._proposal_states[active_idx],
                current_iteration=current_iteration,
            )

            pp_cmds, pp_valid_mask = self._lateral_pure_pursuit_controller(
                initial_velocity_pp, initial_lat_state_pp
            )

            active_steering_rate_cmds = np.zeros(active_idx.size, dtype=CONTROL_DTYPE)
            active_steering_rate_cmds[pp_valid_mask] = pp_cmds[pp_valid_mask]

            if np.any(~pp_valid_mask):
                active_steering_rate_cmds[~pp_valid_mask] = self._lateral_lqr_controller(
                    initial_lateral_state_vector[active_idx][~pp_valid_mask],
                    velocity_profiles[~pp_valid_mask],
                    curvature_profiles[active_idx][~pp_valid_mask],
                )

            steering_rate_cmds[active_idx] = active_steering_rate_cmds
        ...
        command_states = self.refine_command(initial_states, command_states)
    ...
    def  _compute_lookahead_lateral_error(self, 
                                          initial_state: npt.NDArray[np.float64], 
                                          proposal_states: npt.NDArray[np.float64],
                                          current_iteration: SimulationIteration,
                                          lookahead_distance: npt.NDArray[np.float64] = 2,
                                          ) -> npt.NDArray[np.float64]:
        """
        Compute the lateral error at a lookahead distance along the proposal trajectory.
        :param initial_state: The current state for ego.
        :param current_iteration: Used to get the current time.
        :param lookahead_distance: [m] The lookahead distance along the trajectory.
        :return: Lateral error at lookahead point [m].
        """
        assert self._initialized, "BatchLQRTracker: Run update first to load proposal states!"

        # Parameters for lookahead distance
        k_gain_slow = 0.2
        k_gain_fast = 0.2
        speed_threshold = 10.0  # m/s
        Lfc = 2.5

        # Truncate proposals from current time index
        ...
        if Tprime < 2:
            ...
            init_lat = np.stack([
                ...
                initial_state[:, StateIndex.STEERING_ANGLE],
                initial_state[:, StateIndex.ACCELERATION_X]
            ], axis=-1).astype(np.float64)
            return init_vel, init_lat

        # Ego current states
        v0 = initial_state[:, StateIndex.VELOCITY_X]            # [K]
        ego_x = initial_state[:, StateIndex.X]
        ego_y = initial_state[:, StateIndex.Y]
        ego_heading = initial_state[:, StateIndex.HEADING]
        steering_angle = initial_state[:, StateIndex.STEERING_ANGLE]
        a_long = initial_state[:, StateIndex.ACCELERATION_X]

        xy = poses[..., :2]                                     # [K,T',2]

        # Arc-length
        diffs = np.diff(xy, axis=1)                             # [K,T'-1,2]
        step_len = np.linalg.norm(diffs, axis=-1)               # [K,T'-1]
        s_profile = np.concatenate(
            [np.zeros((K, 1), dtype=np.float64), np.cumsum(step_len, axis=1)], axis=1
        )                                                       # [K,T']

        # Lookahead distance per sample
        d_la = np.where(
            v0 < speed_threshold,
            k_gain_slow * np.maximum(v0, 0.0),
            k_gain_fast * np.maximum(v0, 0.0),
        )
        d_la = Lfc + d_la                                       # [K]

        # Row-wise first index where s_profile >= d_la
        cmp = s_profile >= d_la[:, None]                        # [K,T']
        has_hit = cmp.any(axis=1)
        first_hit = np.argmax(cmp, axis=1)                      # [K]
        idx_hi = np.where(has_hit, first_hit, Tprime - 1)
        idx_lo = np.clip(idx_hi - 1, 0, Tprime - 1)

        # Linear interpolation between [idx_lo, idx_hi] on arc-length
        x_lo = xy[np.arange(K), idx_lo, 0]; y_lo = xy[np.arange(K), idx_lo, 1]
        x_hi = xy[np.arange(K), idx_hi, 0]; y_hi = xy[np.arange(K), idx_hi, 1]
        s_lo = s_profile[np.arange(K), idx_lo]; s_hi = s_profile[np.arange(K), idx_hi]
        w = np.clip((d_la - s_lo) / np.maximum(s_hi - s_lo, 1e-6), 0.0, 1.0)
        x_goal = x_lo + w * (x_hi - x_lo)
        y_goal = y_lo + w * (y_hi - y_lo)

        # Transform goal point to ego frame -> (xL, yL)
        cos_h = np.cos(ego_heading); sin_h = np.sin(ego_heading)
        dx = x_goal - ego_x; dy = y_goal - ego_y
        xL = dx * cos_h + dy * sin_h
        yL = -dx * sin_h + dy * cos_h

        # Pack outputs for pure-pursuit controller
        initial_velocities = v0.astype(np.float64)
        initial_lateral_state_vector = np.stack(
            [xL, yL, steering_angle, a_long], axis=-1
        ).astype(np.float64)

        return initial_velocities, initial_lateral_state_vector
    
    def _lateral_pure_pursuit_controller(
        self,
        initial_velocity: npt.NDArray[np.float64],
        initial_lateral_state_vector: npt.NDArray[np.float64],
    ) -> Tuple[npt.NDArray[np.float64], npt.NDArray[np.bool_]]:
        """
        Vectorized Pure Pursuit controller for K proposals (no Python for-loop).
        - Truncate each proposal from closest point to initial_state rear axle.
        - Compute lookahead point by moving a distance d = k*v + Lfc along arc-length.
        - Do linear interpolation between original samples (avoids full resampling).
        - Returns steering_rate for each of K proposals: [K].
        """
        assert self._initialized, "BatchLQRTracker: Run update first to load proposal states!"
        L = self._wheel_base
        dt = self._discretization_time

        xL, yL = initial_lateral_state_vector[:, 0], initial_lateral_state_vector[:, 1]
        Ld = np.hypot(xL, yL)

        alpha = np.arctan2(yL, xL)
        delta_target = np.arctan2(2.0 * L * np.sin(alpha), np.maximum(Ld, 1e-6))

        steering_angle = initial_lateral_state_vector[:, 2]
        ...
        delta_lim = np.arctan(max_abs_yaw_rate * L / v_safe)
        delta_next_min = -delta_lim; delta_next_max = delta_lim
        rr_min = (delta_next_min - sa) / dt
        rr_max = (delta_next_max - sa) / dt
        rate_min = np.minimum(rr_min, rr_max)
        rate_max = np.maximum(rr_min, rr_max)

        a_long = initial_lateral_state_vector[:, 3]

        cos_sa = np.cos(sa)
        sec2 = 1.0 / np.maximum(cos_sa * cos_sa, 1e-3)
        denom2 = (v_safe / L) * sec2
        bias = (a_long / L) * np.tan(sa)
        rr2_min = (-max_abs_yaw_accel - bias) / denom2
        rr2_max = ( max_abs_yaw_accel - bias) / denom2
        rate_min = np.maximum(rate_min, np.minimum(rr2_min, rr2_max))
        rate_max = np.minimum(rate_max, np.maximum(rr2_min, rr2_max))

        steering_rate_cmds = np.clip(base_rate, rate_min, rate_max)
        steering_rate_cmds = np.where(initial_velocity < 1e-1, 0.0, steering_rate_cmds)
        pp_valid_mask = np.logical_or(initial_velocity < 1e-1, Ld > 2.5)

        return steering_rate_cmds, pp_valid_mask
    ...
    def refine_command(self, 
        ...
        steering_rate_cmd = command_states[:, DynamicStateIndex.STEERING_RATE]
        accel_cmd = command_states[:, DynamicStateIndex.ACCELERATION_X]

        initial_steering_angle = initial_states[:, StateIndex.STEERING_ANGLE]
        initial_acceleration = initial_states[:, StateIndex.ACCELERATION_X]
        initial_velocity = initial_states[:, StateIndex.VELOCITY_X]
        initial_yaw_rate = initial_states[:, StateIndex.ANGULAR_VELOCITY]
        initial_angular_acceleration = initial_states[:, StateIndex.ANGULAR_ACCELERATION]
        ...
        max_lon_accel = 2.3  # m/s^2
        min_lon_accel = -4.0 # m/s^2 (strong braking)
        max_abs_lon_jerk = 4.0 # m/s^3

        # lat_lon_acc_shift = self._rear_axle_to_center * (initial_yaw_rate**2 + initial_angular_acceleration)
        # accel_cmd = np.clip(accel_cmd, -3.0 - lat_lon_acc_shift, 2.0 - lat_lon_acc_shift)
        accel_cmd = np.clip(accel_cmd, -3.0, 1.5)

        exceed_limits = np.ones(initial_states.shape[0], dtype=bool)
        revise_value = 0.05 # m/s^2, a small value to ensure convergence when close to limits
        max_revise_time = 20
        revise_time = 0

        # while np.any(exceed_limits) and revise_time < max_revise_time:

        #     next_lon_acc = self.lon_acc(accel_cmd, steering_angle_cmd, initial_states)
        #     next_lon_jerk = self.lon_jerk(next_lon_acc, initial_states)

        #     exceed_accel = next_lon_acc > max_lon_accel
        #     exceed_decel = next_lon_acc < min_lon_accel
        #     exceed_jerk = np.abs(next_lon_jerk) > max_abs_lon_jerk

        #     exceed_limits = exceed_accel | exceed_decel | exceed_jerk
        #     revise_time += 1

        #     if np.any(exceed_limits):
        #         accel_cmd = np.where(
        #             exceed_accel,
        #             accel_cmd - revise_value,
        #             accel_cmd,
        #         )
        #         accel_cmd = np.where(
        #             exceed_decel,
        ...
        #         accel_cmd = np.where(
        #             exceed_jerk,
        #             accel_cmd - np.sign(next_lon_jerk) * revise_value,
        #             accel_cmd,
        #         )
        ...
        refined_command_states = np.array(command_states, copy=True)
        refined_command_states[:, DynamicStateIndex.ACCELERATION_X] = accel_cmd
        refined_command_states[:, DynamicStateIndex.STEERING_RATE] = steering_rate_cmd

        return refined_command_states
    
    def lon_acc(
            self,
            acc_command: npt.NDArray[np.float64],
            steering_angle_cmd: npt.NDArray[np.float64],
            initial_states: npt.NDArray[np.float64],
            ) -> npt.NDArray[np.float64]:
        """This method compute the longitudinal acceleration.
        :param acc_command: The commanded acceleration [m/s^2].
        :param steering_angle_cmd: The commanded steering angle [rad].
        :param initial_states: The current states of the vehicle.
        :return: A boolean array indicating which commands exceed limits.
        """
        next_velocity = initial_states[:, StateIndex.VELOCITY_X] + acc_command * self._discretization_time
        next_yaw_rate =  next_velocity * np.tan(steering_angle_cmd) / self._wheel_base
        next_yaw_acc = (next_yaw_rate - initial_states[:, StateIndex.ANGULAR_VELOCITY]) / self._discretization_time
        next_lon_acc = (
            acc_command 
            + self._rear_axle_to_center * next_yaw_rate**2
            + self._rear_axle_to_center * next_yaw_acc 
        )
        return next_lon_acc
    
    def lon_jerk(
            self,
            next_lon_acc: npt.NDArray[np.float64],
            initial_states: npt.NDArray[np.float64],
            ) -> npt.NDArray[np.float64]:
        
        current_lon_acc = (
            initial_states[:, StateIndex.ACCELERATION_X] 
            + self._rear_axle_to_center * initial_states[:, StateIndex.ANGULAR_VELOCITY]**2
            + self._rear_axle_to_center * initial_states[:, StateIndex.ANGULAR_ACCELERATION] 
        )
        return (next_lon_acc - current_lon_acc) / self._discretization_time