...
class Trajectory(AbstractModelFeature):
    ...
    :param data: either a [num_batches, num_states, s] or [num_states, s] representing the trajectory
    ...
    def to_feature_tensor(self, dtype: torch.dtype = torch.float32) -> Trajectory:
        """Inherited, see superclass."""
        return Trajectory(data=to_tensor(self.data, dtype=dtype))
