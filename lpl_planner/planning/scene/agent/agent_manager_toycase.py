...
class AgentManager:
    ...
    def step_with_planner_input(self, planner_input: PlannerInput, lane_map: LaneMap):

        ego_state , observation = planner_input.history.current_state
        
        tracked_objects = observation.tracked_objects
        self._genenrate_agent_point_set_with_constant_velocity(tracked_objects,
                                                               ego_state,
                                                               lane_map)
        
        occupancy_maps = []
        occupancy_map = self.get_agent_map_from_scenario(ego_state=ego_state)
        occupancy_maps.append(occupancy_map)
    ...
    def _generate_agent_point_set_with_scene(self,
        ...
        agent_boxs = None
        agent_points_radius = None

        static_object_centers = None
        static_points_radius = None

        initial_time_point = ego_state.time_point.time_s

        # if use local coordinates, ignore agent behind ego/ not in route
        if self.use_local_coords:
            ref_path = lane_map.ref_path
            lane_boundaries = lane_map.lane_boundaries
            dist_start = np.zeros(2,) - ref_path[...,:2]
            dist = np.linalg.norm(dist_start, axis=-1)
            nearest_point_idx  = np.argmin(dist,axis= -1)
            ego_s0 = ref_path[nearest_point_idx,3]

        for tracked_object in tracked_objects:

            # ignore agent too far
            agent_dist = np.linalg.norm(ego_state.rear_axle.array 
                                        - tracked_object.center.array)
            if agent_dist > self._map_radius:
                continue
            ...
            if tracked_object.tracked_object_type == TrackedObjectType.VEHICLE:
                ...
                self.agent_dict[tracked_object.token] = tracked_object
                predictions = tracked_object.predictions[0]
                predicted_waypoints = predictions.valid_waypoints
                headings = np.array([waypoint.heading for waypoint in predicted_waypoints])
                agent_centers = np.array([waypoint.center.array for waypoint in predicted_waypoints])
                velocity = tracked_object.velocity.magnitude()
                # headings = np.concatenate(([tracked_object.center.heading],headings),axis=0)
                # agent_centers = np.concatenate((tracked_object.center.array.reshape(1,-1),agent_centers),axis=0)
                
                if agent_centers.shape[0] != self.prediction_sampling.num_poses+1:
                    # raise AssertionError(f'way points not match sampling {agent_centers.shape[0]}/{self.prediction_sampling.num_poses+1}')
                    
                    # Calculate the number of missing poses
                    missing_poses = self.prediction_sampling.num_poses + 1 - agent_centers.shape[0]
                    
                    # Pad agent_centers with the last known position
                    agent_centers = np.pad(agent_centers, ((0, missing_poses), (0, 0)), mode='edge')
                    
                    # Pad headings with the last known heading
                    headings = np.pad(headings, (0, missing_poses), mode='edge')
                    
                
                if self.use_local_coords:
                    ego_yaw = - ego_state.rear_axle.heading
                    agent_centers = agent_centers - ego_state.rear_axle.array
                    agent_centers = np.matmul(agent_centers, np.array([[np.cos(ego_yaw),np.sin(ego_yaw)],
                                                            [-np.sin(ego_yaw),np.cos(ego_yaw)]]))
                    headings = headings + ego_yaw

                    # filter out agents behind ego
                    dist_start = agent_centers[0,:] - ref_path[...,:2]
                    dist = np.linalg.norm(dist_start, axis=-1)
                    nearest_point_idx = np.argmin(dist,axis= -1)
                    s0 = ref_path[nearest_point_idx,3]
                    relative_velocity = velocity * np.cos(headings[0]) - ego_state.dynamic_car_state.rear_axle_velocity_2d.magnitude()
                    if (s0 - ego_s0) < -5 and relative_velocity < 0:
                        # print(f'skip vehicle{tracked_object.token} for behind ego {(s0 - ego_s0)} meter')
                        continue

                    
                agent_states = np.concatenate((agent_centers,headings.reshape(-1,1)),axis=-1)
                
                # create agent box
                half_width = tracked_object.box.half_width + self.lateral_safe_margin
                half_length = tracked_object.box.half_length + self.longitudinal_safe_margin
                
                agent_box = state_array_to_box_array(agent_states,half_length,half_width)
                # agent_box = np.expand_dims(agent_box,axis=0)
                agent_point_radius = np.array([[half_width*0.5,
                                                half_width*0.5,
                                                half_width*0.5,
                                                half_width*0.5,
                                                half_width]]).T
                # agent_point_radius = np.expand_dims(agent_point_radius,axis=0)

                if agent_boxs is None:
                    agent_boxs = agent_box
                    agent_points_radius = agent_point_radius
                else:
                    agent_boxs = np.concatenate((agent_boxs,agent_box),axis=1)
                    agent_points_radius = np.concatenate((agent_points_radius, 
                                                          agent_point_radius),
                                                          axis=0)
            ...
            # 2. pedestrian
                ...
                if self.use_local_coords:
                    ...
                    # ignore pedestrain behind pr out of route
                    dist_start = agent_centers[0,:] - ref_path[...,:2]
                    dist = np.linalg.norm(dist_start, axis=-1)
                    deviation  = np.min(dist,axis= -1)
                    nearest_point_idx = np.argmin(dist,axis= -1)
                    s0 = ref_path[nearest_point_idx,3]
                    nearest_point = ref_path[nearest_point_idx,:2]
                    nearest_point_yaw = ref_path[nearest_point_idx, 2]
                    yaw_metirc = np.vstack((-np.sin(nearest_point_yaw),np.cos(nearest_point_yaw))).T
                    lat_error = np.sum((nearest_point - agent_centers[0,:]) * yaw_metirc,axis=-1)
                    deviation *= -np.sign(lat_error)

                    if (s0 - ego_s0) < 3 or (
                        deviation < lane_boundaries[nearest_point_idx,1] 
                        or deviation > lane_boundaries[nearest_point_idx,0]
                    ):
                        continue



                half_width = tracked_object.box.half_width
                half_length = tracked_object.box.half_length
                
                # create agent box
                agent_box = np.expand_dims(agent_centers, axis=1) # [horizon, 1, 2]
                agent_point_radius = np.ones((1,1)) * max(half_width,half_length) * 1.4
            ...
            # 3. cyclist
                ...
                if self.use_local_coords:
                    ...
                    # ignore cyclists behind or out of route
                    dist_start = agent_centers[0,:] - ref_path[...,:2]
                    dist = np.linalg.norm(dist_start, axis=-1)
                    deviation = np.min(dist,axis= -1)
                    nearest_point_idx = np.argmin(dist,axis= -1)
                ...
                lon_point_num = math.ceil(half_length * 2 / resolution)
                lat_point_num = math.ceil(half_width * 2 / resolution)
                lon_resoluion = half_length * 2 / (lon_point_num + 1)
                lat_resoluion = half_width * 2 / (lat_point_num + 1)

                agent_box = np.zeros(shape=(agent_centers.shape[0], lon_point_num * lat_point_num, 2))
                agent_point_radius = np.ones(shape=(lon_point_num * lat_point_num, 1))*resolution
                
                for t in range(agent_centers.shape[0]):
                    for i in range(lon_point_num):
                        for j in range(lat_point_num):
                            agent_box[t,i * lat_point_num + j,...] = agent_centers[t,:] + \
                                                            np.array([(i+1) * lon_resoluion,
                                                                    (j+1) * lat_resoluion ])
            ...
            # 4. process static objects
            else:
                self.static_object_dict[tracked_object.token] = tracked_object
                static_object_center = tracked_object.box.center.array
                static_point_radius = max(tracked_object.box.half_length,
                                           tracked_object.box.half_width)
                heading = tracked_object.center.heading

                # translate the position to local coordinates
                if self.use_local_coords:
                    ego_yaw = - ego_state.rear_axle.heading
                    static_object_center = static_object_center - ego_state.rear_axle.array
                    static_object_center = np.matmul(static_object_center, np.array([[np.cos(ego_yaw),np.sin(ego_yaw)],
                                                            [-np.sin(ego_yaw),np.cos(ego_yaw)]]))
                    heading = heading + ego_yaw

                    # ignore static obs behind or out of route
                    dist_start = static_object_center - ref_path[...,:2]
                    dist = np.linalg.norm(dist_start, axis=-1)
                    deviation = np.min(dist,axis= -1)
                    nearest_point_idx = np.argmin(dist,axis= -1)
                    s0 = ref_path[nearest_point_idx,3]
                    ...
                    nearest_point_yaw = ref_path[nearest_point_idx, 2]
                    yaw_metirc = np.vstack((-np.sin(nearest_point_yaw),np.cos(nearest_point_yaw))).T
                    lat_error = np.sum((nearest_point - static_object_center) * yaw_metirc,axis=-1)
                    deviation *= -np.sign(lat_error)
                ...
                # add static object to occupancy map
                half_length = tracked_object.box.half_length
                half_width = tracked_object.box.half_width
                resolution = 0.5
                ...
                agent_box = np.zeros(shape=(lon_point_num * lat_point_num, 2))
                agent_point_radius = np.ones(shape=(lon_point_num * lat_point_num, 1))*resolution

                for i in range(lon_point_num):
                    for j in range(lat_point_num):
                        agent_box[i * lat_point_num + j] = translate_lon_and_lat(static_object_center,
                                                                                 heading,
                                                                                (i+1) * lon_resoluion,
                                                                                (j+1) * lat_resoluion)

                agent_box = np.expand_dims(agent_box, axis=0)
                agent_box = np.repeat(agent_box, self.prediction_sampling.num_poses+1, axis=0)
        ...
        # register tensors         
        if agent_boxs is None:
            self.agent_points = torch.ones(size=(self.prediction_sampling.num_poses+1,1,2), **self._tenser_args)*-10
            self.point_radius = torch.ones(size=(1,1), **self._tenser_args)*0.1
        else:
            self.agent_points = torch.tensor(agent_boxs, **self._tenser_args)
            # agent_points: [horizon, point_num,2]
            self.point_radius = torch.tensor(agent_points_radius, **self._tenser_args)
            # agent_points: [point_num,1]

        # self.static_points = torch.tensor(static_object_centers, **self._tenser_args)
        # self.static_radius = torch.tensor(static_points_radius, **self._tenser_args)


    def _genenrate_agent_point_set_with_constant_velocity(self,
                                             tracked_objects: List[TrackedObject],
                                             ego_state: EgoState = None,
                                             lane_map: LaneMap = None):
        ...
        for tracked_object in tracked_objects:
            ...
            if tracked_object.tracked_object_type == TrackedObjectType.VEHICLE:
                ...
                self.agent_dict[tracked_object.token] = tracked_object
                current_center = tracked_object.center.array
                velocity_vector = np.array([tracked_object.velocity.x, tracked_object.velocity.y])
                num_steps = self.prediction_sampling.num_poses + 1
                time_intervals = np.arange(num_steps) * self.time_step
                agent_centers = current_center + velocity_vector * time_intervals.reshape(-1, 1)
                headings = np.full(num_steps, tracked_object.center.heading)
                
                # Pad if needed (should not be necessary with correct num_steps)
                if agent_centers.shape[0] != self.prediction_sampling.num_poses+1:
                    missing_poses = self.prediction_sampling.num_poses + 1 - agent_centers.shape[0]
                    agent_centers = np.pad(agent_centers, ((0, missing_poses), (0, 0)), mode='edge')
                    headings = np.pad(headings, (0, missing_poses), mode='edge')
                ...
                if self.use_local_coords:
                    ...
                    # Filter agents behind ego
                    dist_start = agent_centers[0,:] - ref_path[...,:2]
                    dist = np.linalg.norm(dist_start, axis=-1)
                    nearest_point_idx = np.argmin(dist, axis=-1)
                    s0 = ref_path[nearest_point_idx,3]
                    relative_velocity = np.linalg.norm(velocity_vector) * np.cos(headings[0]) - ego_state.dynamic_car_state.rear_axle_velocity_2d.magnitude()
                    if (s0 - ego_s0) < -5 and relative_velocity < 0:
                        continue
                ...
                half_width = tracked_object.box.half_width + self.lateral_safe_margin
                half_length = tracked_object.box.half_length + self.longitudinal_safe_margin
                agent_box = state_array_to_box_array(np.column_stack((agent_centers, headings)), half_length, half_width)
                agent_point_radius = np.array([[half_width]*4 + [half_width*0.5]]).T

                # Concatenate to agent_boxs
                if agent_boxs is None:
                    agent_boxs = agent_box
                    agent_points_radius = agent_point_radius
                else:
                    agent_boxs = np.concatenate((agent_boxs, agent_box), axis=1)
                    agent_points_radius = np.concatenate((agent_points_radius, agent_point_radius), axis=0)
            ...
            # 2. pedestrian
                ...
                # Local coordinate transform and filtering
                if self.use_local_coords:
                    ego_yaw = - ego_state.rear_axle.heading
                    agent_centers = agent_centers - ego_state.rear_axle.array
                    agent_centers = np.matmul(agent_centers, np.array([[np.cos(ego_yaw),np.sin(ego_yaw)],
                ...
                # Create points and radius
                half_width = tracked_object.box.half_width
                half_length = tracked_object.box.half_length
                agent_box = np.expand_dims(agent_centers, axis=1)
                agent_point_radius = np.ones((1,1)) * max(half_width, half_length) * 1.4
            ...
            # 3. bicycle (similar to pedestrian)
            elif tracked_object.tracked_object_type == TrackedObjectType.BICYCLE:
                self.agent_dict[tracked_object.token] = tracked_object
                current_center = tracked_object.center.array
                velocity_vector = np.array([tracked_object.velocity.x, tracked_object.velocity.y])
    ...
    def get_static_obstacles_feature(self,
        ...
        # 检查 static_object_feature 是否有非法值（NaN 或 Inf），如有则打印变量名与对应位置
