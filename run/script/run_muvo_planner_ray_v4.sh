...
ANCHOR_ROOT=$RESULTS_DIR/planner_anchors
LOG_ROOT=$RESULTS_DIR/simulation_logs
mkdir -p "$CKPT_ROOT" "$ANCHOR_ROOT" "$LOG_ROOT" "$RESULTS_DIR/rollout_data"
SIM_OUTPUT_DIR=""
DEVICE="auto"  # or "auto" | "cpu" | "cuda:1"
USE_HAUSDORFF=false  # whether to use hausdorff distance in scoring
HAUSDORFF_WEIGHT=0.1  # weight for hausdorff distance in scoring
USE_ANCHOR_VELOCITY=false  # whether to use anchor velocity in scoring


PLANNER=muvo_abstract_planner_v4
USE_EVAL=false  # whether to use trajectory evaluator to evaluate the trajectories
NUM_SAMPLES=32  # number of trajectory samples to generate
NUM_GPU=0.04  # number of GPU to allocate per simulation
TOP_K=0.0 # number of top trajectories to consider
TOP_P=0.0 # cumulative probability for nucleus sampling
PRED_LOGPROB_WEIGHT=1.0  # weight for prediction loss in scoring
EVAL_LOGPROB_WEIGHT=0.0  # weight for evaluation loss in scoring
SCORE_CHUNK_SIZE=512  # chunk size for scoring anchors in inference (reduce if OOM, increase if under-utilized GPU)
PREDICTION_MODE='CYAW'  # 'CV', 'CA', 'CYAW', 'prediction'
EVAL_SAMPLING_DT=0.2  # sampling interval for evaluation, only used when use_eval is true

INTERMEDIATE_DIM=256  # hidden dimension for the model

USE_PREDICTION=true  # whether to use agent prediction
...
FILTER=test14-hard # 'val14', 'val14-reduced', 'test14-random', 'test14-hard', 'test14-hard-test'
WORKER=${WORKER:-custom_ray_distributed}
WORKER_THREADS_PER_NODE=${WORKER_THREADS_PER_NODE:-64}
...
MAX_SPEED_DIFF=20 # maximum speed difference for dynamic scoring
...
CKPT=${CKPT:-"pm_muvo_v4_t4_4096_full_ap_h4s_30_lw_anchor_score_softce02/last.ckpt"}  

# CKPT="mv_cl_single_agem_task_mem24000_r005/task_00_single_increment/last.ckpt"  # CL single increment ckpt
JOB_NAME=pm_muvo_v4_t4_4096_full_ap_h4s_30_lw_anchor_score_softce02_cyaw_32_fusedeval_M_p1e0c0_lqrdiv_nolatrefine/$FILTER
VIDEO_SAVE_DIR=$RESULTS_DIR/simulation_videos/$CHALLENGE/$JOB_NAME
SAVE_REPLAY=true  # whether to save replay video during simulation
SAVE_NUBOARD_DATA=false  # no dedicated run_simulation_ray flag exists; when false, prune job artifacts after aggregation
NUM_POSES=20

SAVE_ROLLOUT_DATA=false  # whether to save rollout data for future training
ROLLOUT_CACHE_DIR=${ROLLOUT_CACHE_DIR:-"$RESULTS_DIR/rollout_data"} # path to save rollout data
SIM_OUTPUT_DIR="$LOG_ROOT/$CHALLENGE/$JOB_NAME"
...
python ./run/simulation/run_simulation_ray.py \
    ...
    planner.muvo_planner.ckpt_path="$CKPT_ROOT/$CKPT" \
...
if [[ "$SAVE_NUBOARD_DATA" != "true" && -d "$SIM_OUTPUT_DIR" ]]; then
    python - "$SIM_OUTPUT_DIR" <<'PY'
from pathlib import Path
import shutil
import sys
